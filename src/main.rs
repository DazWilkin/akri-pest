...
fn good() {
    ...
    let successful_parse = ZeroConfParser::parse(Rule::full_domain, &full_domain);
...
fn bad() {
    let unsuccessful_parse = ZeroConfParser::parse(Rule::protocol, "dns");
...
mod tests {
    ...
    #[test]
    fn test_kind() {
        let kind: String = format!("_{}._{}", RUST, TCP);
        parses_to! {
            parser:ZeroConfParser,
            input:&kind,
            rule:Rule::kind,
